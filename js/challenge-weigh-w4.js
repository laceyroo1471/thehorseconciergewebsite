...
(function () {
  ...
  var CONTEST_ID = 'weigh-wednesday-w4';
  var FIELD = 'weighWednesdayW4';
  var STORAGE_DISMISS = 'thcWeighW4Dismissed_v1';
  var PENDING_KEY = 'thcWeighW4PendingGuesses';
  var OPEN_MS = Date.parse('2026-09-23T06:00:00-04:00');
  var CLOSE_MS = Date.parse('2026-09-27T20:00:00-04:00');
  ...
  var ITEMS = [
    {
      ...
      group: 'Hay storage',
      detail:
        'About how many pounds can be lost when the outer four inches of a 500-pound round bale deteriorate outdoors?',
      unit: 'lb',
    },
    {
      id: 'annualHorseManure',
      group: 'Barn output',
      detail: 'About how many pounds of manure and urine does one 1,000-pound horse produce in one year?',
      unit: 'lb',
    },
    {
      id: 'monthlyShoppingMiles',
      group: 'Everyday travel',
      detail: 'About how many miles per month does the average American travel for shopping and personal errands?',
      unit: 'miles',
    },
    {
      id: 'oneRatFeedLoss',
      group: 'Feed storage',
      detail: 'In one year, about how many pounds of grain can a single rat eat or contaminate?',
      unit: 'lb',
    },
  ];
  ...
  var preview = params.get('weighW4Preview') === '1';
  if (params.get('weighW4Reset') === '1') {
    try {
  ...
  function itemRowsHtml() {
    return ITEMS.map(function (item, idx) {
      return (
        ...
        item.id +
        '-lb" min="0" max="1000000" step="1" inputmode="numeric" required>' +
        '</label>' +
        '</div>' +
        '</div>'
      );
  ...
  function ensureDialog() {
    dialog = document.getElementById('thc-dialog-weigh-w4');
    if (dialog) {
      form = dialog.querySelector('#weigh-w4-form');
      statusEl = dialog.querySelector('#weigh-w4-status');
      return dialog;
    }
    dialog = document.createElement('dialog');
    dialog.id = 'thc-dialog-weigh-w4';
    dialog.className = 'thc-dialog thc-dialog--challenge thc-dialog--weigh';
    dialog.innerHTML =
      '<div class="thc-dialog__inner thc-dialog__inner--challenge">' +
      '<button type="button" class="thc-dialog-close" data-weigh-w4-dismiss aria-label="Close">&times;</button>' +
      ...
      '<button type="button" class="btn-ghost" data-weigh-w4-dismiss>Not now</button>' +
      '</div>' +
      '</form>' +
      '</div>' +
      '</div>';
  ...
  function setFormLocked(locked) {
    ...
    var submit = form.querySelector('[data-weigh-w4-submit]');
    if (submit) {
      submit.hidden = !!locked;
      submit.textContent = hasSubmitted ? 'Update my guesses' : 'Save my 4 guesses';
    }
    var dismissBtn = form.querySelector('[data-weigh-w4-dismiss]');
    if (dismissBtn) dismissBtn.textContent = locked ? 'Close' : 'Not now';
  }

  function updateCards() {
    document.querySelectorAll('[data-weigh-w4-card]').forEach(function (card) {
      if (windowNotOpenYet() || cardRetired()) {
        card.hidden = true;
        return;
      }
      card.hidden = false;
      var title = card.querySelector('[data-weigh-w4-title]');
      var note = card.querySelector('[data-weigh-w4-note]');
      var openBtn = card.querySelector('[data-weigh-w4-open]');
      ...
      if (hasSubmitted) {
        ...
        if (note) {
          note.textContent =
            'Mid-week bonus · up to 9 points. Guess four hidden barn and household costs. Closest overall wins.';
        }
        if (openBtn) {
          openBtn.hidden = false;
  ...
  function handleSubmit(e) {
    ...
    if (!guesses) {
      setStatus('Enter a number for all four questions.', true);
      return;
    }
    setStatus('Saving your four guesses…');
    var payload = {};
    payload[FIELD] = {
      contestId: CONTEST_ID,
  ...
  function wire() {
    if (!document.querySelector('[data-weigh-w4-card]')) return;
    ensureDialog();
    updateCards();

    dialog.querySelectorAll('[data-weigh-w4-dismiss]').forEach(function (el) {
    ...
    document.querySelectorAll('[data-weigh-w4-open]').forEach(function (btn) {
